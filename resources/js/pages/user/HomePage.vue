    ...
    <div class="container mx-auto px-4 py-8">
        <div class="flex flex-col md:flex-row gap-8">
            <div class="content flex-1 bg-white p-6 rounded-lg shadow-md">
                <header>
                    <h1 class="text-3xl font-bold text-gray-800">Hey, {{ user?.name || 'User' }}!</h1>
                    <div class="credential text-center my-4">
                        <p class="text-gray-600">Use these credentials when you log in again, save them for future
                            reference.</p>
                        <h4 class="text-lg font-semibold">Your Username: {{ user?.username }}</h4>
                        <h4 class="text-lg font-semibold">Password: {{ user?.temp_password || 'Not available' }}</h4>
                    </div>
...
const { data: user, loading: userLoading, error: userError } = useUserApi(userId);
...
const shareLink = computed(() => {
    ...
    const encodedUsername = btoa(user.value?.username || '');
